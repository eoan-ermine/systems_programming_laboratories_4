...
\begin{tikzpicture}[
    ...
    \node [base, join] (OUTSIDE) {OUTSIDE = «Точка лежит вне заштрихованной области»};
    \node [base, join] (INSIDE) {INSIDE = «Точка лежит в заштрихованной области»};
    \node [base, join] (BORDERS) {BORDERS = «Точка лежит на границе заштрихованной области»};
    \node [io, join] (input) {Ввод X, Y};
    \node [base, join] (RSquare) {RSquare = X * X + Y * Y};

    \node [decision, join] (if1) {RSquare > 1};
    \node [io, right of = if1, node distance = 7cm, yshift=-1cm] (if1_true) {Вывод OUTSIDE};
    \path [line] (if1) -| node [u,near start] {Да} (if1_true);
    \node [decision, left of = if1, node distance = 5cm, yshift=-2cm] (if1_false) {$Y >= 0 \ \&\& \ Y <= 0.8$};
    \path [line] (if1) -| node [u,near start] {Нет} (if1_false);

    \node [io, left of = if1_false, node distance = 0 cm, yshift=-3cm] (if2_false) {Вывод OUTSIDE};
    \node [decision, right of=if1_false, node distance = 5 cm, yshift=-2cm] (if2_true) {RSquare == 1 || \\ Y == 0 || Y == 0.8};
    \path [line] (if1_false) -| node [u,near start] {Да} (if2_true);
    \path [line] (if1_false) to node [r] {Нет}(if2_false);

    \node [io, right of = if2_true, node distance = 5 cm, yshift=-4cm] (if3_true) {Вывод \\ BORDERS};
    \path [line] (if2_true) -| node [u, near start] {Нет}(if3_true);
    \node [io, left of = if2_true, node distance = 0 cm, yshift=-2cm] (if3_false) {Вывод INSIDE};
    \path [line] (if2_true) to node [r] {Нет}(if3_false);

    \node [block, join, yshift=-2cm] (finish) {Конец main()};
    \path [line] (if1_true) |- (finish);
    \path [line] (if2_false) |- (finish);
    \path [line] (if3_true) |- (finish);
\end{tikzpicture}
